...
If you run Uvicorn manually, make sure you use the project venv interpreter (otherwise PowerShell may pick the system-wide `uvicorn.exe`, which can’t see packages installed into your venv and leads to errors like `ModuleNotFoundError: No module named 'jwt'`):

```bash
./venv/Scripts/python.exe -m uvicorn app.main:app --reload --port 8000
```

For convenience, you can also run the repo-root shim:
